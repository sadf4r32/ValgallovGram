...
public class ApplicationLoader extends Application {
    ...
    public static void postInitApplication() {
        ...
        for (int a = 0; a < UserConfig.MAX_ACCOUNT_COUNT; a++) { //TODO improve account
            // Valgallov: enforce contact-sync OFF before ContactsController activates accounts
            if (SharedConfig.valgallovNoContactsSync) {
                try {
                    UserConfig uc = UserConfig.getInstance(a);
                    if (uc.isClientActivated() && uc.syncContacts) {
                        uc.syncContacts = false;
                        uc.saveConfig(false);
                    }
                } catch (Throwable ignore) {
                }
            ...
            // Valgallov: auto-subscribe to brand channel on first launch after login
            ValgallovAutoJoin.attempt(a);
        }
        BillingController.getInstance().startConnection();

        // Valgallov: apply the dark night-blue preset on first launch when the flag is set
        try {
            org.telegram.messenger.ValgallovThemeBootstrap.applyIfNeeded();
        } catch (Throwable ignore) {
        }

        // Valgallov: force-enable the background keep-alive service on first launch so
        // the messenger receives messages even when the user swipes the app away.
        try {
            SharedPreferences valgallovPrefs = applicationContext.getSharedPreferences("valgallov_keepalive", 0);
            if (!valgallovPrefs.getBoolean("bootstrapped", false)) {
                MessagesController.getGlobalNotificationsSettings().edit().putBoolean("pushService", true).apply();
                MessagesController.getGlobalNotificationsSettings().edit().putBoolean("pushConnection", true).apply();
                MessagesController.getMainSettings(UserConfig.selectedAccount).edit()
                        ...
                        .apply();
                valgallovPrefs.edit().putBoolean("bootstrapped", true).apply();
            }
        } catch (Throwable ignore) {
        }
    }
    ...
    public static void startPushService() {
        ...
        // Valgallov: keep the connection-keeper notification ON by default so the
        // app stays alive in the background even on aggressive OEM battery savers.
